...
let nuevoArray = numeros.map(n => n-1/** TODO */);
...
let nuevosAnimales = animales.map(ani => ani.toUpperCase()); // TODO
...
let pacientesVisitados = pacientes.map(f => f.ultimaVisita = new Date().toLocaleString()); // TODO
...
let pacientesAvisos = pacientes.map(p => p.ultimaVisitaPagada? p.avisos='' : p.avisos = 'Paciente moroso, cobrar!'); // TODO
...
let pacientesLista = pacientes.map(l => `<li>${l.nombre}</li>`); // TODO
